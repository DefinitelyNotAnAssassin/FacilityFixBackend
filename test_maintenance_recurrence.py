...
class RecurrenceTestRunner:
    ...
    async def setup_test_data(self):
        ...
        try:
            ...
            if not success or not admin_doc:
                ...
                admin_role = admin_doc.get("role")
                print(f"  Admin role: {admin_role}")
                if admin_role != "admin":
                    print(f"  ⚠ Warning: Admin role is '{admin_role}', updating to 'admin'")
    ...
    async def mark_task_completed(self):
        ...
        try:
            ...
            # Verify admin users can be found before recurrence
            print("\nVerifying admin user lookup before checking recurrence...")
            try:
                from app.services.notification_manager import notification_manager
            ...
            # Wait for recurrence logic to execute
            print("\nWaiting 5 seconds for recurrence logic to complete...")
    ...
    async def verify_recurring_task_created(self):
        ...
        try:
            ...
            if recurring_tasks:
                ...
                # Check inventory reservation IDs on the task itself
                if hasattr(recurring_task, 'inventory_reservation_ids'):
                    res_ids = recurring_task.inventory_reservation_ids or []
                    print(f"  Inventory Reservation IDs on task: {len(res_ids)}")
    ...
    async def check_notifications_sent(self):
        ...
        try:
            ...
            # If no notifications found with both filters, try with just entity_id
            if not success or not notifications:
                print(f"No notifications found with both filters, trying with just entity_id...")
                success, notifications, error = await database_service.query_documents(
                    COLLECTIONS["notifications"],
                    [
                        ("related_entity_id", "==", self.recurring_task_id),
            ...
            if success and notifications:
                ...
                for notif in notifications:
                    ...
                    # Staff could be notified by Firebase UID or staff_id
                    if "assigned" in title.lower():
                        if self.test_staff_uid in recipient or "STAFF-REC-001" in message:
                            staff_notifs.append(notif)
                            print(f"    ✓ Staff assignment notification")
                    
                    # Check if it's for admin (recurring task created)
                    if "recurring" in title.lower():
                        # Any recipient that's not the test staff
                        if recipient != self.test_staff_uid:
                            admin_notifs.append(notif)
                            print(f"    ✓ Admin recurring task notification")

                # Also check if admin was notified (even if title doesn't say "recurring")
                for notif in notifications:
                    recipient = notif.get("recipient_id")
                    if recipient == self.test_admin_uid and notif not in admin_notifs:
                        admin_notifs.append(notif)
                        print(f"\n  Found admin notification (by recipient):")
                ...
                if staff_notifs:
                    ...
                    print(f"   Checking all notifications for task...")
                    # Check ALL notifications for this task to see if any exist
                    all_success, all_notifs, _ = await database_service.query_documents(
                        COLLECTIONS["notifications"],
                        [("related_entity_id", "==", self.recurring_task_id)]
                    )
                    if all_success:
                        print(f"   Total notifications for task: {len(all_notifs or [])}")
                ...
                if admin_notifs:
                    ...
                    print(f"   Expected admin UID: {self.test_admin_uid}")
                    print(f"   Notification recipients: {[n.get('recipient_id') for n in notifications]}")
                ...
                # Try to find ANY notifications to debug
                print(f"\n  Debugging: Checking for any notifications...")
                all_success, all_notifs, _ = await database_service.query_documents(
                    COLLECTIONS["notifications"], None
                )
                if all_success and all_notifs:
                    recent_notifs = sorted(all_notifs, key=lambda x: x.get('created_at', ''), reverse=True)[:5]
                    print(f"  Recent notifications in system:")
                    for n in recent_notifs:
                        print(f"    - {n.get('title')} -> {n.get('recipient_id')} (entity: {n.get('related_entity_id')})")
    ...
    async def validate_inventory_reservations(self):
        ...
        try:
            # First check if the task has inventory_reservation_ids
            recurring_task = await maintenance_task_service.get_task(self.recurring_task_id)
            if recurring_task and hasattr(recurring_task, 'inventory_reservation_ids'):
                res_ids = recurring_task.inventory_reservation_ids or []
                if res_ids:
                    print(f"✓ Recurring task has {len(res_ids)} inventory_reservation_id(s) attached")
                    
                    # Fetch each reservation by ID
                    reservations = []
                    for res_id in res_ids:
                        success, res_doc, _ = await database_service.get_document(
                            COLLECTIONS["inventory_reservations"], res_id
                        )
                        if success and res_doc:
                            reservations.append(res_doc)
                    
                    if reservations:
                        print(f"✓ Successfully fetched {len(reservations)} reservation document(s)")
                        
                        for res in reservations:
                            res_id = res.get('id') or res.get('_doc_id')
                            inv_id = res.get('inventory_id')
                        ...
                        print(f"\n  Checking for match:")
                        print(f"    Expected inventory_id: {expected_item}")
                        ...
                        for res in reservations:
                            inv_id = res.get('inventory_id')
                            qty = res.get('quantity')
                            print(f"    Comparing: '{inv_id}' == '{expected_item}' and {qty} == 2")
                            # Match by exact ID or if test inventory ID is in the inventory_id
                            if (inv_id == expected_item or expected_item in str(inv_id)) and qty == 2:
                                found_match = True
                                print(f"    ✓ Match found!")
                                break
                        ...
                        if found_match:
                            ...
                            print(f"   Expected: {expected_item} x 2")
                            print(f"   Found: {[(r.get('inventory_id'), r.get('quantity')) for r in reservations]}")
                    else:
                        print(f"⚠ Could not fetch reservation documents")
                else:
                    print(f"⚠ Recurring task has no inventory_reservation_ids")
            
            # Fallback: Query inventory reservations for the recurring task
            print(f"\nFallback: Querying reservations by maintenance_task_id...")
    ...
    async def run_all_tests(self):
        ...
        try:
            ...
            # Auto-cleanup (no user input required)
